...
const Node: React.FC<{ icon: React.ReactNode, title: string, subtitle: string, highlight?: boolean }> = ({ icon, title, subtitle, highlight = false }) => (
    <div className="flex flex-col items-center text-center w-32 relative z-10">
        <div className={`relative p-4 rounded-xl border bg-slate-900/80 backdrop-blur-sm transition-all duration-300 ${highlight ? 'border-cyan-500/50 shadow-[0_0_20px_-5px_rgba(6,182,212,0.3)]' : 'border-slate-700/50'}`}>
            {icon}
        </div>
